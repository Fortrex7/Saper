...
//rekursivnaya funkciya otkrytiia kletki (proverka sosednih)
...
void end_game(bool is_win = false) {
    ...
    print_field();
    cout << "Vi " << (is_win ? "viigrali" : "proigrali") << ", \nchtoby nachat' novuiu igru vvedite lubuiu stroku: \n";
    string s;
    cin >> s;
}
...
{
    while (true) {
        srand(time(0)); //obnulenie randoma koordinat
        //obnulenie polya
        for (int i = 0; i < N; i++) {
            for (int j = 0; j < M; j++) {
                field[i][j] = 0;
                field_p[i][j] = -2; //zapolnenie nacal'nogo polia
            }
        }
        //generatsiya min (K par koordinat)
        for (int i = 0; i < K; ++i) {
            while (true) {
                int x = rand() % N;
                int y = rand() % M;
                //ustanovka miny na pole
                if (field[x][y] != -1) {
                    field[x][y] = -1;
                    break;
                }
        ...
        //prisvaivanie ostal'nym keltkam chisel s kolichestvom min
        for (int i = 0; i < N; i++) {
            for (int j = 0; j < M; j++) {
                if (field[i][j] != -1) {
                    field[i][j] = 0;
